...
/*  m->generate();
    ...
    //Project 2 code */
    ProblemSolver * PS = new ProblemSolver(SP, B, SP->getDegree());				//Computes k, Md and finds if eigenproblem is standard or generalised
    PS->Solve();										//fint if system can be solved and solve it
    backSubstituteSols(Bp1, Bp2, PS);								//Compute polynomials for computed solutions of Solve()
    changeOfVariable(Bp1, Bp2, SM, PS, B);							//Change of variable as asked in 4)
//    delete result;
    cleanResources(PS, SM, Bp1, Bp2, SP, m);							//clean resources
}
...
bool backSubstituteSols(BivariatePolynomial * Bp1, BivariatePolynomial * Bp2, ProblemSolver * PS)//Compute polynomials for computed solutions
{
    ...
    for (int i = 0; i < numSols; ++i) {								//For every solution
        ...
        if(sol->getMultiplicity() == 1) {
            ...
            if(res1 < 0.000001 && res1 < 0.000001)						//Are the values over 10^-6
...
void changeOfVariable(BivariatePolynomial * Bp1, BivariatePolynomial * Bp2, SylvesterMatrix * SM, ProblemSolver * PS, int B)
{												//Change of variable as askrd in 4)
