...
int SylvesterMatrix::max(int d1, int d2)
{
    if (d1>d2)
        return d1;
    else
...
void SylvesterMatrix::getRandom(int & t)
{
...
void SylvesterMatrix::initMatrixWithHiddenY(BivariatePolynomial *Bp1, BivariatePolynomial *Bp2)         //Make the sylvester matrix with x as the hidden variable
{
    ...
    for (int i = d1 - 1; i >= 0; --i) {
        int until = (d1-1)-i;
        for (int j = 0; j < ColDimension - until; ++j) {
            //if j column beyond the degx assign zero
            if(j>degx1)
            {
