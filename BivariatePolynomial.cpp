...
using namespace std;
//implementation file
void BivariatePolynomial::InitDegs()                            //initialize polynomial y, x degrees
{
    this->degy = 0;
...
{
    cout<<"DEGY : "<<this->degy<<endl<<"DEGX : "<<this->degx<<endl;
    for(int i=matrixDimension-1;i >= 0;i--)
    {
        //cout<<"row "<<i<<" :  ";
        for (int j = matrixDimension - 1; j >= 0; --j)
...
BivariatePolynomial::BivariatePolynomial(char polstr[], int sD)
{
    ...
    for (int i = 0; i < matrixDimension; ++i) {
        ...
        for (int j = 0; j < matrixDimension; ++j) {
            this->MatrixRepresentation[i][j] = 0;                   //initialize the matrix
        }
    }
    Parse(polstr);                                              //parse the polynomial
    InitDegs();                                         //find the degrees
}
BivariatePolynomial::BivariatePolynomial(int sD)
{
    ...
    for (int i = 0; i < matrixDimension; ++i) {
        this->MatrixRepresentation[i] = new int[matrixDimension];
        for (int j = 0; j < matrixDimension; ++j) {
            this->MatrixRepresentation[i][j] = (rand()%100) - 50;
        }
    }
    InitDegs();
}
...
void BivariatePolynomial::ParsePolynomialBody(int & i, char * polstr, char first, char second, int & firstCoord, int & secondCoord, int & choice, int coef)       //parsing method
{
...
{
    int count = 0;
    if(!isdigit(polstr[pos]))       //if no digit return 1
    {
        pos += count - 1;
        return 1;
    }
...
//        delete this->MatrixRepresentation[i];
//    }
